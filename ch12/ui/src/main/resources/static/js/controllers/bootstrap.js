...
/* Controllers */
  ...
  ;
app.controller('headerController', ['$state', '$localStorage', '$scope', '$http',function($state, $localStorage, $scope, $http) {
  $scope.realName = $localStorage.userinfo.realName;
  $scope.headerImg = '';
  $scope.headerImgUrl = '';
  var url = '/getFiles/'+$localStorage.userinfo.conductssId+'/head';
    ...
    $http.get('/flows/getTaskNum/').success(function(data){
      if(data.data.length != 0){
        for(var i in data.data){
          $scope.taskNum += data.data[i].taskNum;
        }
      }
    });
  }
