	  ...
	  .controller('MwlCalendarCtrl', ["$scope", "$log", "$timeout", "$attrs", "$locale", "moment", "calendarTitle", function($scope, $log, $timeout, $attrs, $locale, moment, calendarTitle) {
	    ...
	    function refreshCalendar() {
	      ...
	      if (calendarTitle[vm.view] && angular.isDefined($attrs.viewTitle)) {
	        if (vm.view === 'month') {
	          //$log.info(vm.currentDay);
	          vm.viewTitle = moment(vm.currentDay).format('YYYY年MM月');
	        }else {
	    ...
	    $scope.$watchGroup([
	      ...
	      if (!eventsWatched) {
	        ...
	        $scope.$watch('vm.records', refreshCalendar, true);
	        $scope.$watch('vm.tips', refreshCalendar, true);
	      } else {
	        refreshCalendar();
	      }
	    });
