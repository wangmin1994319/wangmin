    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "       <th ng-repeat=\"col in colDefinitions\" ng-style=\"{width: {{col.width}} + 'px'}\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          "     <tr ng-mouseenter=\"mouse_enter(row.branch)\" ng-mouseleave=\"mouse_leave(row.branch)\" ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
          "       ng-click=\"user_clicks_branch(row.branch)\" ng-class=\"'level-' + {{ row.level }} + (row.branch.selected ? ' gridSelected':'')\" class=\"tree-grid-row\">\n" +
          "       <td><a ><i ng-class=\"row.tree_icon\"\n" +
          "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              class=\"indented tree-icon\">&nbsp;<i ng-class=\"row.org_icon\"></i></i>\n" +
          "           </a><span class=\"indented tree-label\" ng-click=\"on_user_click(row.branch)\">\n" +
          "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +
          "       </td>\n" +
          "       <td ng-repeat=\"col in colDefinitions\" ng-style=\"{height: {{col.height}} + 'px'}\">\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            attrs.iconLeaf = 'text-info icon icon-user';
            attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'fa fa-plus-square-o';
            attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'fa fa-minus-square-o';
            attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : 'text-info icon icon-user';
            ...
            scope.mouse_enter = function(branch){
              branch.mouseEnter = true;
            };
            scope.mouse_leave = function(branch){
              branch.mouseEnter = false;
            };
            /* sorting methods */
            ...
            on_treeData_change = function () {
              ...
              add_branch_to_list = function (level, branch, visible) {
                var child, child_visible, tree_icon, _i, _len, _ref, _results,org_icon;
                if (branch.expanded == null) {
                  branch.expanded = false;
                }
                if(branch.type == 0){
                  org_icon = "fa fa-building-o text-info";
                }else if(branch.type == 3){
                  org_icon = "fa fa-building-o text-info";
                }else if(branch.type == 1){
                  org_icon = "fa icon-screen-tablet text-info";
                }
                if (!branch.children || branch.children.length === 0) {
                  if(branch.type == 0 || branch.type == 1) {
                    if (branch.expanded) {
            ...
            if (scope.treeControl != null) {
              if (angular.isObject(scope.treeControl)) {
                ...
                tree.get_root_branch = function (b) {
                  while(null != tree.get_parent_branch(b)){
                ...
                tree.add_branch_list = function (parent, new_branch) {
                  if (parent != null) {
                    for(var i in new_branch){
                      parent.children.push(new_branch[i]);
                      new_branch[i].expanded = true;
                    }
                    parent.expanded = true;
                  }
                  return new_branch;
